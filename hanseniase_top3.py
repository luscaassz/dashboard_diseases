...
INPUT_FILE = "data/for_model/df_base_TX_hanseniase_00_23.csv"
OUTPUT_BASE_DIR = "results/prophet_hanseniase_custom"
FORECAST_MONTHS = 160 
...
def prepare_data_hanseniase(df, cod_mun):
...
def main():
    ...
    print(f"Iniciando Modelagem de Hanseníase com: {TOP_REGRESSORS}")
    df_full = pd.read_csv(INPUT_FILE)
    municipios = df_full['cod_mun'].unique()
    
    for cod in tqdm(municipios, desc="Processando Municípios"):
        df_mun = prepare_data_hanseniase(df_full, cod)
